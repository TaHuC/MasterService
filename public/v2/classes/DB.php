...
//            }
    ...
    private function actionAll($action, $table) {
        ...
        $sqlNum = "{$action} FROM {$table}";
            if($this->_query = $this->_pdo->query($sqlNum)){
                $this->_countNum = $this->_query->rowCount();
            } else {
                $this->_error = true;
            }
    ...
    private function actionPagination($action, $table, $limit, $end) {

        $sqlNum = "{$action} FROM {$table} ORDER BY id DESC LIMIT $limit, $end";

        if ($this->_query = $this->_pdo->query($sqlNum)) {
            if ($this->_query->execute()) { // sledkato mine conekciqta se sluchva tazi stypka
                $this->_resultsPagination = $this->_query->fetchAll();
                $this->_count = $this->_query->rowCount();
    ...
    public function getPagination($table, $limit, $end) {
        return $this->actionPagination('SELECT *', $table, $limit, $end);
    }
    ...
    public function resultPagination(){
        return $this->_resultsPagination;
    }
    ...
    public function countNum() { // izkarva rezultata
